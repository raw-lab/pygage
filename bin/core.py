...
class GAGEPreparation:
    ...
    def prepare_expression(
        ...
        """Return a gene_col + per-comparison fold-change matrix.
...
class GAGEAnalysis:
    ...
    def run_gage(
        ...
        """Run GAGE. Returns dict with ``greater`` (+``less`` if same_dir) and ``stats``.

        Parity: default (t-test + stouffer + BH) reproduces gage R.
        Extensions: ``meta_method='fisher'``; ``control_genes`` (background from a
        control set rather than all genes); ``global_bh`` (BH across greater∪less);
        ``compute_effect`` (mean fold-change per set); ``leading_edge`` (member
        genes driving the signal); ``permutations`` (sample-label permutation
        null p-value); ``n_jobs`` (parallel over gene sets).
        """
        ...
        schema = {
            "gene_set": pl.Utf8, "set_size": pl.Int64, "stat_mean": pl.Float64,
            "p_geomean": pl.Float64, "p_greater": pl.Float64, "p_less": pl.Float64,
        }
        if not rows:
